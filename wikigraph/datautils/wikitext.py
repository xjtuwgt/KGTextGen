...
# `wikitext-103-raw`.
DATA_ROOT = join(HOME_DATA_FOLDER, 'wikitext-103')
...
def articles_from_file(file_path: str) -> List[WikitextArticle]:
    ...
    # Skip the first part which is empty as range start from 1
...
class RawDataset(dataset.Dataset):
    ...
    def _load_data(self):
        ...
        if self._dataset is None:
            ...
            logging.info('Loading data from {}'.format(f'{data_root}/wiki.{self._subset}.{self._version}'))
            self._dataset = articles_from_file(
                f'{data_root}/wiki.{self._subset}.{self._version}')

        def source():
            n_articles = len(self._dataset)
            logging.info('Loading {} articles'.format(n_articles))
