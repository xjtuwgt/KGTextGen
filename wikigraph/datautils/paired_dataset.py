...
from wikigraph.datautils import dataset
from wikigraph.datautils import io_utils
from wikigraph.datautils import tokenizers
from wikigraph.datautils import tools

ArrayType = Union[np.ndarray]
DATA_ROOT = join(HOME_DATA_FOLDER, 'wikigraphs')
...
class RawDataset(dataset.Dataset):
    ...
    def _load_data(self):
        ...
        if self._dataset is None:
            self._dataset = list(io_utils.read_pairs_from_gzip_txt_file(
                f'{self._data_dir}/{self._version}/{self._subset}.gz'))

        def source():
            n_pairs = len(self._dataset)
            logging.info('Loading {} pairs'.format(n_pairs))
...
class Graph:
    ...
    @classmethod
    def subsample_nodes(cls, graph: 'Graph', subsample_rate: float = 1.0, center_node: str = None) -> 'Graph':
...
class BaseGraph2TextDataset(dataset.Dataset):
    ...
    def _process_graph_text_pair(
        ...
        return (self._process_graph(pair.center_node, pair.graph),
                self._tokenizer.encode(pair.text, prepend_bos=True, append_eos=True))
